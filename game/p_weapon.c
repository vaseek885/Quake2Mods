...
======================================================================
*/
void weapon_trilauncher_fire(edict_t *ent)
{
			...
			vec3_t angle;
			VectorSet(angle, ent->client->v_angle[0], ent->client->v_angle[1]+15, ent->client->v_angle[2]-30);
			AngleVectors(angle, forward, right, NULL);
			P_ProjectSource(ent->client, ent->s.origin, offset, forward, right, start);
			VectorScale(forward, -2, ent->client->kick_origin);
			
			ent->client->kick_angles[0] = -1;
			fire_grenade(ent, start, forward, damage, 600, 2.0, radius);
			//-left
			//center
			VectorSet(offset, 0, 0, ent->viewheight - 8);
			VectorSet(angle, ent->client->v_angle[0], ent->client->v_angle[1], ent->client->v_angle[2]);
			AngleVectors(angle, forward, right, NULL);
			P_ProjectSource(ent->client, ent->s.origin, offset, forward, right, start);
			VectorScale(forward, -2, ent->client->kick_origin);
			...
			//right
			VectorSet(offset, 0, 0, ent->viewheight - 8);
			VectorSet(angle, ent->client->v_angle[0], ent->client->v_angle[1]-15, ent->client->v_angle[2]);
			AngleVectors(angle, forward, right, NULL);
			P_ProjectSource(ent->client, ent->s.origin, offset, forward, right, start);
			VectorScale(forward, -2, ent->client->kick_origin);
...
void Weapon_TriLauncher(edict_t *ent)
{
	static int	pause_frames[] = { 34, 51, 59, 0 };
	static int	fire_frames[] = { 6, 0 };
	Weapon_Generic(ent, 5, 16, 59, 64, pause_frames, fire_frames, weapon_trilauncher_fire);
}
...
void LM_Fire(edict_t *ent)
{
	...
	for (i = 1; i<3; i++)
	{
		ent->client->kick_origin[i] = crandom() * 0.60;
		ent->client->kick_angles[i] = crandom() * 0.8;
	}
	ent->client->kick_origin[0] = crandom() * 0.60;
	ent->client->kick_angles[0] = ent->client->machinegun_shots * -1.5;
	...
	VectorSet(offset, 0, 8, ent->viewheight - 8);
	P_ProjectSource(ent->client, ent->s.origin, offset, forward, right, start);
	fire_rail(ent, start, forward, damage * 2, kick, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MOD_MACHINEGUN);
...
void Weapon_LM(edict_t *ent)
{
	static int	pause_frames[] = { 23, 45, 0 };
	static int	fire_frames[] = { 4, 5, 0 };

	Weapon_Generic(ent, 3, 5, 45, 49, pause_frames, fire_frames, LM_Fire);
}
